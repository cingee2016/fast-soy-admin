      ...
      <div class="flex items-center gap-12px">
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-4px flex gap-8px text-11px text-gray-400">
            ...
            <span>
              {{ $t('page.manage.radar.monitor.threads') }}: {{ (realtime?.cpu || overview?.cpu)?.threads || 0 }}
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-12px">
        ...
        <div class="min-w-0 flex-1">
          <div class="mb-4px flex items-center justify-between">
            ...
            <span class="ml-8px shrink-0 text-16px font-bold">
              {{ (realtime?.memory || overview?.memory)?.usage || 0 }}%
            </span>
          </div>
          <NProgress
            type="line"
          ...
          <div class="mt-4px flex gap-8px text-11px text-gray-400">
            <span>
              {{ $t('page.manage.radar.monitor.used') }}: {{ (realtime?.memory || overview?.memory)?.used || 0 }}GB
            </span>
            <span>
              {{ $t('page.manage.radar.monitor.total') }}: {{ (realtime?.memory || overview?.memory)?.total || 0 }}GB
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-12px">
        ...
        <div class="min-w-0 flex-1">
          <div class="mb-4px flex items-center justify-between">
            ...
            <span class="ml-8px shrink-0 text-16px font-bold">
              {{ (realtime?.disk_io || overview?.disk_io)?.usage || 0 }}%
            </span>
          </div>
          <NProgress
            type="line"
          ...
          <div class="mt-4px flex gap-8px text-11px text-gray-400">
            <span>
              {{ $t('page.manage.radar.monitor.used') }}: {{ (realtime?.disk_io || overview?.disk_io)?.used || 0 }}GB
            </span>
            <span>
              {{ $t('page.manage.radar.monitor.total') }}: {{ (realtime?.disk_io || overview?.disk_io)?.total || 0 }}GB
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-12px">
        ...
        <div class="min-w-0 flex-1">
          <div class="mb-4px flex items-center justify-between">
            ...
            <span class="ml-8px shrink-0 text-16px font-bold">
              {{ (realtime?.network_io || overview?.network_io)?.active_connections || 0 }}
            </span>
          </div>
          <div class="flex flex-col gap-2px text-12px">
            <div class="flex justify-between">
              ...
              <span class="font-medium">
                {{ (realtime?.network_io || overview?.network_io)?.upload_speed || '0 B/s' }}
              </span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-400">{{ $t('page.manage.radar.monitor.download') }}</span>
              <span class="font-medium">
                {{ (realtime?.network_io || overview?.network_io)?.download_speed || '0 B/s' }}
              </span>
            </div>
          </div>
        </div>
