...
from app.utils import BizError, Code, Fail, StateMachine, Success, assert_object_policy, create_system_user, emit, get_current_user_id, get_db_conn, radar_log
...
async def _create_employee_core(emp_in: EmployeeCreate, redis):
    ...
    # 系统用户和员工资料必须一起成功；不要在事务内做额外外部 IO。
    async with in_transaction(get_db_conn(Employee)):
        ...
        # 用户字段只喂给 create_system_user，Employee 只保留业务资料和 user_id 绑定。
...
async def get_subordinate_or_fail(mgr: Employee, emp_id: int) -> Employee:
    """Return a subordinate in the manager's department or raise a business error."""
    # 主管入口共用这一道门，避免 team API 每个动作各写一份部门归属判断。
...
async def update_subordinate_employee(mgr: Employee, emp_id: int, emp_in: EmployeeUpdate):
    ...
    await get_subordinate_or_fail(mgr, emp_id)
    return await update_employee(emp_id, emp_in)


async def transition_subordinate(mgr: Employee, emp_id: int, to_state: str):
    """主管推进下属状态。"""
    await get_subordinate_or_fail(mgr, emp_id)
...
async def get_public_showcase_overview() -> dict:
    ...
    # 公开展示只返回聚合数据，不暴露手机号、邮箱、工号或 user_id。
