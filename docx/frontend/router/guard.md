...
每次路由跳转都会经过 `web/src/router/guard/` 的守卫链：认证 → 动态路由加载 → 权限校验 → 进度条 / 标题副作用。
...
```typescript
meta: { roles: ['R_ADMIN', 'R_HR_MANAGER'] }
```
...
登录后做额外初始化（拉字典 / 拉用户偏好）时，在 `route.ts` 的 `initRoute` 里加一步即可。**不建议**新增独立的 `router.beforeEach`——容易和现有守卫的执行顺序冲突。
