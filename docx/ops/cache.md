...
启动时 `refresh_all_cache(redis)` 一次性把所有数据加载进去。CUD 时**增量**刷新——例如修改某个角色后调 `await load_role_permissions(redis, role_code="R_HR_MANAGER")`，否则用户继续看缓存的旧权限。
...
```bash
# 查看角色权限缓存
redis-cli get "role:R_HR_MANAGER:apis" | jq
...
- [RBAC](../develop/rbac.md) — 权限缓存怎么被 RBAC 用
- [认证](../develop/auth.md) — token_version 用法
- [启动初始化与对账](../develop/init-data.md) — `refresh_all_cache` 何时跑
